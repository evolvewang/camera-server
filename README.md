...
并通过 ZeroMQ PUB/SUB 对外提供低延迟视频流。color 流发布为 JPEG，depth 流发布为
无损 16 位 PNG；客户端分别还原为 OpenCV BGR `uint8` 和原始 `uint16` 数组。
...
`pyorbbecsdk` 及其本机运行库需要按照 Orbbec SDK 的方式安装，并确保设备驱动可用。
...
或其他订阅程序检查 color/depth 的持续接收和解码。
